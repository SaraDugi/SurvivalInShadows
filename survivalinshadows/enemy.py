...
class Enemy(Entity):
    def __init__(self,enemy_type,name, pos, groups, obstacle_sprites):
        ...
        self.pathfinding_methods = {
                    'astar': astar,
                    'geneticalal': find_best_path,
                    'bfs': bfs_with_safety_check,
                    'decisiontree': decision_tree_pathfinding,
                    'knn': pathfinding,
                    'default': bfs_with_safety_check
                }
    ...
    def enemy_move(self, player):
        ...
        if self.enemy_type == 'geneticalal':
        ...
        if self.enemy_type == 'bfs':
            path = bfs_with_safety_check(maze, start, end)
            if path is not None and len(path) > 0:
                ...
                self.direction = pygame.math.Vector2()
        if self.enemy_type == 'decisiontree':
        ...
        if self.enemy_type == 'knn':
            path = pathfinding(maze, start, end, k=3)
            if path is not None and len(path) > 0:
                next_step = list(path[0])
                next_step[0] *= TILESIZE
                next_step[1] *= TILESIZE
